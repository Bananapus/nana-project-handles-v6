...
- A read for `(chainId, projectId, setter=X)` can only return name parts that address `X` itself wrote. No other address can poison `X`'s slot. Setter slots are isolated by the `_msgSender()` storage key (`src/JBProjectHandles.sol:130`).
- A malicious or broken ENS resolver can only **hide** an otherwise-resolvable handle (forcing `handleOf` to return `""`) — it cannot make `handleOf` return an attacker-controlled string or another setter's stored parts. The low-level staticcall to the resolver (`src/JBProjectHandles.sol:287-315`) soft-fails on failure, malformed ABI, or oversized response.
- A name-owner-controlled resolver cannot grief readers with unbounded copy costs: `_MAX_TEXT_RECORD_LENGTH = 256` (`src/JBProjectHandles.sol:62, 306`) caps the manual byte copy. `chainId:projectId` is always well under that bound.
- Reads against chains without ENS deployed (`address(ENS_REGISTRY).code.length == 0`) return `""` instead of reverting (`src/JBProjectHandles.sol:185`). Multi-chain frontends will not break on non-mainnet calls.
...
## C.1 `constructor(address trustedForwarder)` — `src/JBProjectHandles.sol:81`
...
## C.2 `setEnsNamePartsFor(uint256 chainId, uint256 projectId, string[] memory parts)` — `src/JBProjectHandles.sol:96-135`
...
## C.3 `ensNamePartsOf(uint256 chainId, uint256 projectId, address setter) view → string[]` — `src/JBProjectHandles.sol:146-157`
...
## C.4 `handleOf(uint256 chainId, uint256 projectId, address setter) view → string` — `src/JBProjectHandles.sol:165-205`
...
- **Invariants:**
  ...
  - returns `""` for chains without ENS deployed (`code.length == 0` check at `:185`)
...
- **`_formatHandle(parts) pure → string`** — reverses parts and joins with `"."` (e.g. `["jbx","dao","foo"] → "foo.dao.jbx"`). Used by both write-emit and read-verify, so the visible handle string is deterministic from storage (`src/JBProjectHandles.sol:214-226`).
- **`_namehash(parts) pure → bytes32`** — EIP-137 namehash with an implicit trailing `"eth"` suffix label (`src/JBProjectHandles.sol:244-265`). Iterates the formatted handle right-to-left at dot boundaries.
- **`_slice(input, start, end) pure → bytes`** — pure substring (`src/JBProjectHandles.sol:272-278`).
- **`_textRecordOf(textResolver, hashedName) view → string`** — low-level staticcall to `ITextResolver.text`; rejects failed calls, non-standard ABI offset (`!= 32`), oversized length (`> result.length - 64` or `> _MAX_TEXT_RECORD_LENGTH`); soft-fails to `""` in all failure paths (`src/JBProjectHandles.sol:284-316`).
- **`_msgSender() / _msgData()`** — ERC-2771 forwarder-aware overrides (`src/JBProjectHandles.sol:230-238`). Determines the storage-key identity for writes.
...
4. **Soft-fail everywhere on the read path.** Every ENS-side failure mode (no registry on this chain, no resolver, resolver reverts, ABI-malformed return, oversized return, wrong text record) collapses to `handleOf == ""`. Reads cannot revert under hostile resolver behavior.

5. **Resolver gas-griefing is bounded.** A name-owner-controlled resolver could return a maximally large valid ABI string forcing readers to copy bytes before the equality check rejects them. The `_MAX_TEXT_RECORD_LENGTH = 256` cap bounds that work (`src/JBProjectHandles.sol:62, 306`).
...
- Storage key (setter isolation): `src/JBProjectHandles.sol:73-74, 130, 156, 176`
- Label validation (empty, `"eth"`, control bytes, DEL, dots): `src/JBProjectHandles.sol:101-127`
- ENS registry constant: `src/JBProjectHandles.sol:47`
- Text-key constant (`"juicebox"`): `src/JBProjectHandles.sol:50`
- ERC-2771 forwarder wiring: `src/JBProjectHandles.sol:18, 81, 230-238`
- `handleOf` read flow + soft-fail branches: `src/JBProjectHandles.sol:165-205`
- ENS-not-deployed soft-fail: `src/JBProjectHandles.sol:185`
- Text-record cap (`_MAX_TEXT_RECORD_LENGTH = 256`): `src/JBProjectHandles.sol:62, 306`
- Low-level resolver staticcall with ABI hardening: `src/JBProjectHandles.sol:284-316`
- EIP-137 namehash with implicit `"eth"` suffix: `src/JBProjectHandles.sol:244-265`
- Event for indexer reconstruction: `src/JBProjectHandles.sol:132-134` and `src/interfaces/IJBProjectHandles.sol:19-21`
